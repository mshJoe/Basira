...
def calculate_alert(forecast_df, historical_df) -> dict:
    ...
    if risk_probability > 0.70 or (worst_day_value < 0 and days_to_worst <= 18):
        color = "red"
        message = f"خطر سيولة متوقع خلال {days_to_worst} يوماً — تصرف فوراً"
    elif risk_probability > 0.40 or worst_day_value < 2000:
        color = "yellow"
        message = "ضغط على السيولة محتمل خلال 30 يوماً — راقب الوضع"
    else:
        color = "green"
        message = "السيولة مستقرة — لا مخاوف خلال الشهر القادم"
    
    reasons = detect_reasons(historical_df)
...
def detect_reasons(df) -> list:
    ...
    if len(df) >= 60:
        ...
        if expense_change > 0.10:
            reasons.append(f"ارتفاع تكاليف التشغيل بنسبة {round(expense_change*100)}% هذا الشهر")
        
        sales_change = (last_30['sales'].mean() - prev_30['sales'].mean()) / prev_30['sales'].mean()
        if sales_change < -0.10:
            reasons.append(f"انخفاض المبيعات بنسبة {round(abs(sales_change)*100)}% مقارنة بالشهر الماضي")
    
    reasons.append("موعد دفع الإيجار والرواتب خلال أسبوعين")
